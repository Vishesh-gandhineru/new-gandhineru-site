...
export function PrimaryButton({ children, href , className , icon }: PrimaryButtonProps) {
 
 
  return (
      <Button variant="Primary" asChild className={`relative group primaryButton mr-[40px] before:bg-gradient-to-b after:bg-gradient-to-t from-transparent from-80% to-50% ${className}` }>
        <Link href={href ? href : "#"}>
          <span>{children}</span>
